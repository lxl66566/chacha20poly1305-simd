...
class Plot:
    ...
    def text(self, x, y, s, size=12.0, anchor="middle", color="#333", mono=False,
             rotate=None):
    ...
    def frame(self, xticks, yticks, yfmt, xlabel="", ylabel=""):
        ...
        if xlabel:
            self.text((self.ml + self.w - self.mr) / 2, self.h - 8, xlabel, size=12, color="#444")
        if ylabel:
            self.text(
                18, (self.mt + self.h - self.mb) / 2, ylabel, size=12, color="#444",
                rotate=-90,
            )
...
def chart_throughput():
    ...
    p.frame(SIZES, yticks, fmt_mib, xlabel="message size", ylabel="throughput (seal, in-place)")

    for t in TIERS:
        p.line(SIZES, series[f"rc-{t}"], COLOR[t], dash="6 4", width=1.8, opacity=0.55,
               marker=False)
    p.line(SIZES, series["openssl"], OSSL_COLOR, dash="10 4 2 4", width=2.6, marker=True)
    for t in TIERS:
        p.line(SIZES, series[f"ours-{t}"], COLOR[t], width=2.4)

    p.text(p.ml + 6, p.mt - 12,
           "ChaCha20-Poly1305 seal throughput — this crate (solid) vs RustCrypto "
           "(dashed, same ISA tier) vs OpenSSL 4.1-dev (dash-dot, auto-dispatch)",
           size=13, anchor="start", color="#222")

    # legend
    lx, ly = p.ml + 14, p.mt + 16
    entries = []
    for t in TIERS:
        entries.append((f"this crate · {TIER_LABEL[t]}", COLOR[t], None))
    entries.append(("RustCrypto · same tier (dashed)", "#777", "6 4"))
    entries.append(("OpenSSL 4.1-dev · auto (AVX-512/IFMA)", OSSL_COLOR, "10 4 2 4"))
    p.el.append(
        f"<rect x='{lx - 8}' y='{ly - 14}' width='300' height='{len(entries) * 20 + 12}' "
        f"fill='white' stroke='#ccc' opacity='0.92'/>"
    )
    for i, (label, c, d) in enumerate(entries):
        yy = ly + i * 20
        dd = f" stroke-dasharray='{d}'" if d else ""
        p.el.append(
            f"<line x1='{lx}' y1='{yy - 4}' x2='{lx + 28}' y2='{yy - 4}' stroke='{c}' "
            f"stroke-width='2.4'{dd}/>"
        )
        p.text(lx + 36, yy, label, size=11.5, anchor="start", color="#333")
...
def chart_speedup():
    ...
    series["openssl"] = [
        DATA["openssl"]["auto"][i] / DATA["ours"]["avx512"][i] for i in range(len(SIZES))
    ]
    ally = [y for ys in series.values() for y in ys]
    ...
    p.frame(SIZES, yticks, lambda v: f"{v:g}×", xlabel="message size",
    ...
    p.line(SIZES, series["openssl"], OSSL_COLOR, dash="10 4 2 4", width=2.4, marker=True)

    p.text(p.ml + 6, p.mt - 12,
           "Speedup: this crate vs RustCrypto (per ISA tier) and vs OpenSSL 4.1-dev "
           "(our AVX-512 vs their auto)",
           size=13, anchor="start", color="#222")
    ...
    p.el.append(
        f"<rect x='{lx - 8}' y='{ly - 14}' width='240' height='{len(entries) * 20 + 12}' "
        f"fill='white' stroke='#ccc' opacity='0.92'/>"
    )
    for i, (label, c, d) in enumerate(entries):
        yy = ly + i * 20
